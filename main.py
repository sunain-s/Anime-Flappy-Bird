...
def draw_pipes(pipe_list, pipe_surface):
    for pipe in pipe_list:
...
def score_display(game_state, score, highscore):
...
def run():
    ...
    while True:
        ...
        if game_active:
            ...
            draw_pipes(pipe_list, pipe_surface)

            # score
            score = pipe_score_check(score, can_score, pipe_list, score_sound )
            score_display('main_game', score, highscore)
        else:
            timer -= 1
            screen.blit(game_over_surface, game_over_rect)
            highscore = update_score(score, highscore)
            score_display('game_over', score, highscore)
